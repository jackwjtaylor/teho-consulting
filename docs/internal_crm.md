...
- **Follow-up radar** – Highlights companies whose last outreach event is older than a configurable window (default 3–5 days depending on status). Includes a manual event form so you can log follow-ups, replies, meetings, etc. into `outreach_events` directly from `/admin`.  
- **Portal access** – Lists Supabase auth users, shows their current `client_slug`/role metadata, and lets you assign/revoke access via the same portal (calls `auth.admin.updateUser`).  
